...
# df = df.groupby("day_id").filter(lambda x: len(x) >= max_encoder_length + max_prediction_length)

# day_ids_sorted = sorted(df['day_id'].unique())
# test_days = day_ids_sorted[-5:]
# test_df = df[df['day_id'].isin(test_days)].copy()

# # --- Normalizer ---
# normalizer = GroupNormalizer(groups=["day_id"], transformation="softplus", center=False)
# normalizer.fit(df["Glucose Value (mg/dL)"], df)

# # --- TimeSeriesDataSet for Test ---
...
# # --- Load best model from W&B artifact ---
# wandb.login()
...
# print("Saved test_predictions_with_truth.csv")
